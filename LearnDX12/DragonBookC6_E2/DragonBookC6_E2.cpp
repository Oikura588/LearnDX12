...
void BoxApp::Draw(const GameTimer& gt)
{
    ...
    // 两个Slot绑定.因为Color不会变，应该可以只绑定一次.
    mCommandList->IASetVertexBuffers(0,1,&mBoxGeo->VertexBufferView()); //POSITION
    mCommandList->IASetVertexBuffers(1,1,&mBoxGeo->VertexBufferView()); //COLOR
...
void BoxApp::BuildShadersAndInputLayout()
{
    ...
    D3D12_INPUT_ELEMENT_DESC PositionInput = {"POSITION",0,DXGI_FORMAT_R32G32B32_FLOAT,0,0,D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0};
    D3D12_INPUT_ELEMENT_DESC ColorInput = {"COLOR",0,DXGI_FORMAT_R32G32B32A32_FLOAT,1,0,D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0};

    mInputLayout =
    {
        PositionInput,ColorInput
    };
}
